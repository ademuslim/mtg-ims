...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["add"])) {
        ...
        $query_check_email = "SELECT * FROM data_pelanggan WHERE email = ?";
    ...
    if (isset($_POST["edit"])) {
        // Ambil nilai yang dikirim dari form
        $idPelanggan = $_POST["id_pelanggan"]; // ID pelanggan yang akan diedit
        $nama = strtolower($_POST["nama"]);
        $alamat = strtolower($_POST["alamat"]);
        $noTelp = $_POST["no_telp"];
        $email = strtolower($_POST["email"]);
        ...
        $conditions = "id_pelanggan='$idPelanggan'";
        $customMessage = "pelanggan";
